...
void launch_iters(Tile_t **rvec_tile_array, Tile_t **pvec_tile_array, Tile_t **xvec_tile_array, Replay_Tile_t **Apvec_tile_array,
		  ...
		  const failure_generator_t * failgen_ptr)
{
  ...
  for (int tt=0; tt < n_iters; ++tt){
	...
	futures->push_back( pvec_tile_array[tt][i].promise->get_future() );
	  ...
	  //default task has succeeded unless marked to fail
	  *succeeded = true;
	  if(fails) { 
	    int idx = 0;
	    idx = hclib::resilience::replay::get_index();
	    if(idx < fails) {*succeeded = false; }
	  }
...
int main (int argc, char *argv[])
{
 ...
 Replay_Tile_t** Apvec_tile_array = (Replay_Tile_t **) malloc(sizeof(Replay_Tile_t*)*(n_iters+1));
 ...
 for (int tt=0; tt < n_iters+1; ++tt) {
   ...
   Apvec_tile_array[tt] = (Replay_Tile_t *) malloc(sizeof(Replay_Tile_t)*(n_tiles));
...
// #else
	 ...
	 = new hclib::resilience::replay::promise_t<DoubleVec*>(2); //Accessed only for (1) the dot product, (2) r_next = r - alpha * Ap	  
//#endif
