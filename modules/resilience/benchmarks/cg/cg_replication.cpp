...
void launch_iters(Tile_t **rvec_tile_array, Tile_t **pvec_tile_array, Tile_t **xvec_tile_array, Diamond_Tile_t **Apvec_tile_array,
		  ...
		  int n_tiles, int n_iters, const std::vector<spmv_tile> * Amat_row_tiles_ptr,
		  const failure_generator_t * failgen_ptr)
{
  ...
  for (int tt=0; tt < n_iters; ++tt){
    ...
    for (int j=0; j<n_tiles; ++j){
      //Injecting failures only in the SPMV task
      //Does this task_idx fail
      int fails = failgen_ptr->should_fail(1+j + tt*n_tiles);
	...
	futures->push_back( pvec_tile_array[tt][i].promise->get_future() );
	  ...
	  //Inject failure if task-replica is deemedto have failed
	  int tilesize = workspace->size();
	  if(fails) { 
	    int idx = 0;
	  ...
	  if (res == 0){
	    fprintf(stderr,"Fatal error: resilience failed to save you on it %d tile %d\n", tt, j);
	    exit(EXIT_FAILURE);
	  }
...
int main (int argc, char *argv[])
{
 ...
 FILE *pFile = fopen(failfile.c_str(),"r");
 if (pFile == NULL){
 ...
 Diamond_Tile_t** Apvec_tile_array = (Diamond_Tile_t **) malloc(sizeof(Diamond_Tile_t*)*(n_iters+1));
 ...
 for (int tt=0; tt < n_iters+1; ++tt) {
   ...
   Apvec_tile_array[tt] = (Diamond_Tile_t *) malloc(sizeof(Diamond_Tile_t)*(n_tiles));
...
// #else
	 ...
	 = new hclib::resilience::diamond::promise_t<DoubleVec*>(2); //Accessed only for (1) the dot product, (2) r_next = r - alpha * Ap	  
//#endif
 ...
 hclib::finish([rvec_tile_array, pvec_tile_array, xvec_tile_array, Apvec_tile_array,
		...
		alpha_iter, beta_iter, n_tiles, n_iters, Amat_row_tiles_ptr, &failgen]() {
