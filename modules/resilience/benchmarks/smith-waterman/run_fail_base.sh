if [ $# -ne 3 ]; then
    echo "USAGE: ./run.sh <WORKLOAD> <APP> <ERR>"
...
if [ "$SIZE" == "tiny" ]; then
    ...
    TILE_WIDTH=1450
    TILE_HEIGHT=1500
    $HWLOC_CMD ./$2 -input1 ${INPUT_FILE_1} -input2 ${INPUT_FILE_2} -tile_width ${TILE_WIDTH} -tile_height ${TILE_HEIGHT} -nochecksum -inject_rate $3
