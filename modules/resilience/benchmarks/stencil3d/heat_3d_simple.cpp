...
// This is standard 3D stencil with six neighbors. This code is used
// to analyze performance degradations introduced by the chosen
...
typedef struct {
#if defined(USE_REPLAY)
  hclib::resilience::replay::promise_t<Tile3D*>* promise;
#elif defined(USE_REPLICATION)
  hclib::resilience::diamond::promise_t<Tile3D*>* promise;
#else
  hclib::ref_count::promise_t<Tile3D*>* promise;
#endif
  ...
  hclib::launch(deps, 1, [&]() {
    ...
    for (int_t tt=0; tt < params.n_steps+1; ++tt) {
      ...
      for (int_t j=0; j < totaltiles; ++j) {
        ...
        tile_array[tt][j].promise
            = new hclib::resilience::replay::promise_t<Tile3D*>(
                tt < params.n_steps ? 7 : 1);
#elif defined(USE_REPLICATION)
        tile_array[tt][j].promise
            = new hclib::resilience::diamond::promise_t<Tile3D*>(
                tt < params.n_steps ? 7 : 1);
#else
        tile_array[tt][j].promise
            = new hclib::ref_count::promise_t<Tile3D*>(
                tt < params.n_steps ? 7 : 1);
#endif
    ...
    hclib::finish([tile_array, &params, &cfl, &failgen]() {
      launch_iters(tile_array, params.n_tiles, params.n_steps,
                   cfl, params.do_checksums, failgen);
    });
