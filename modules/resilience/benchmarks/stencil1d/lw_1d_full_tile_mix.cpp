...
// redundant work.  This code allows mixing resilience mechanisms.
...
typedef struct {
  hclib::resilience::promise_t<DoubleVec*>* promise;
...
inline
bool advance_tile_body(const Tile_t &left_tile_prev,
                       const Tile_t &right_tile_prev,
                       const Tile_t &center_tile_prev,
                       const Tile_t &center_tile_updated,
                       int_t n_steps_task, double cfl,
                       bool do_checksums, int fails
#if defined(DUMP_TIMINGS) || defined(DUMP_DURATION)
                      ,bool do_timing
#endif
                      )
                  ...
                  resilience_type_t task_strategy)
{
  ...
  if (task_strategy == replay_resilient){ /* use replay */
    hclib::resilience::replay::async_await_check<LEAF,3>([=]{
      bool pass = advance_tile_body(left_tile_prev, right_tile_prev,
                                    center_tile_prev, center_tile_updated,
                                    n_steps_task, cfl, do_checksums,
                                    fails
#if defined(DUMP_TIMINGS) || defined(DUMP_DURATION)
                                   ,do_timing
#endif
    ...
    hclib::resilience::diamond::async_await_check<LEAF>([=]{
      advance_tile_body(left_tile_prev, right_tile_prev,
                        center_tile_prev, center_tile_updated,
                        n_steps_task, cfl, false, fails  /* forces no checksums on replication */
#if defined(DUMP_TIMINGS) || defined(DUMP_DURATION)
                       ,do_timing
#endif
                       );
...
int main (int argc, char *argv[])
{
  ...
  hclib::launch(deps, 1, [&]() {
    ...
    resilience_strategy_t strategy(params.n_tiles, n_rounds, params.replic_frac);
    ...
    hclib::finish([tile_array, &params, &n_rounds, &cfl, &failgen,
      ...
      launch_iters(tile_array, params.n_tiles, n_rounds,
                   params.n_steps_task, cfl, params.do_checksums,
                   failgen, strategy);
    });
