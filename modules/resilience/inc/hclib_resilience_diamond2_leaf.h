...
inline void async_await_at_leaf(T&& lambda, hclib_locale_t *locale) {
    ...
    hclib::async_nb_await_at( [=]() {
        *(hclib_get_curr_task_local()) = dtp;
        (*lambda_ptr)();
        delete lambda_ptr;
        auto task_local = static_cast<diamond_task_params_t<void*>*>(dtp);
        int count = __sync_sub_and_fetch(task_local->count, 1);
        if(count == 0)
            task_local->finish_prom->put(1);
...
async_await_check_at(T&& lambda, hclib::promise_t<int> *prom_check,
    ...
    hclib::async_await_at([=]() {
        ...
        auto mpi_send_vec = new mpi_data_vector();
#endif

        auto count = new int(N);
        auto finish_prom = new hclib::promise_t<int>();
...
#ifdef USE_RESILIENT_PROMISE
                dtp_arr[i].mpi_send_vec = mpi_send_vec;
#endif
                dtp_arr[i].count = count;
                dtp_arr[i].finish_prom = finish_prom;
                *(hclib_get_curr_task_local()) = &dtp_arr[i];
                async_await_at_leaf(*lambda_ptr, locale);
            }

        hclib_future_wait(finish_prom->get_future());
