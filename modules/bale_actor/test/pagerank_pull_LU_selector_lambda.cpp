...
// 
 *****************************************************************/ 
/*! \file pagerank_pull_LU_selector_lambda.cpp
...
double calculate_curr_iter_pagerank(sparsemat_t* L, double* page_rank, double* page_rank_curr_iter, int64_t* degrees_total) {
    hclib::Selector<2> *pagerankSelector = new hclib::Selector<2>();
    ...
    hclib::finish([=]() {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                ...
                pagerankSelector->send(0, L->lnonzero[k] % THREADS, [=]() {
                    double pr_y = page_rank[src_idx];
                    double deg_y = (double)(get_mat()->loffset[src_idx+1] - get_mat()->loffset[src_idx]);
                    int64_t dest_idx_selector = dest_idx;

                    pagerankSelector->send(1, pe_dest, [=]() {
                        page_rank_curr_iter[dest_idx_selector] += pr_y / (double)deg_y;               
