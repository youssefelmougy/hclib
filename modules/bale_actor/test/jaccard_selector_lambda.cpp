...
// 
 *****************************************************************/ 
/*! \file jaccard_selector_lambda.cpp
...
double get_edge_degrees(sparsemat_t* L, int64_t* edge_degrees) {
    hclib::Selector<2> *degreeSelector = new hclib::Selector<2>();
    ...
    hclib::finish([=]() {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                ...
                int64_t i = L->lnonzero[k] / THREADS;
                int64_t src_idx = pos;
                int64_t pe_src = MYTHREAD;

                degreeSelector->send(0, L->lnonzero[k] % THREADS, [=]() {
                    int64_t src_idx_selector = src_idx;
                    int64_t i_selector = get_mat()->loffset[i + 1] - get_mat()->loffset[i];

                    degreeSelector->send(1, pe_src, [=]() {
                        edge_degrees[src_idx_selector] = i_selector;
                    });
                });
...
double jaccard_selector(sparsemat_t* A2, double* jaccard_index, int64_t* edge_degrees) {
    ...
    hclib::Selector<2> *jacSelector = new hclib::Selector<2>();
        ...
        for (int64_t v = 0; v < A2->lnumrows; v++) {             //vertex v (local)
            for (int64_t k = A2->loffset[v]; k < A2->loffset[v+1]; k++) {
                ...
                for (int64_t kk = A2->loffset[v]; kk < A2->loffset[v+1]; kk++) {
                    ...
                    if (x == u) continue;
                    int64_t index_u = u / THREADS;
                    int64_t pe_src = MYTHREAD;

                    jacSelector->send(0, u % THREADS, [=]() {
                        for (int64_t uk = get_mat()->loffset[index_u]; uk < get_mat()->loffset[index_u+1]; uk++) {
                            if (x == get_mat()->lnonzero[uk]) {
                                int64_t pos_selector = pos;
                                jacSelector->send(1, pe_src, [=]() {
                                    jaccard_index[pos_selector]++;
                                });
                            }
                        }
                    });
